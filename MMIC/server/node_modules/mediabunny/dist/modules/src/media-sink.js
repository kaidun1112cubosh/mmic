...
import { addAvcBitstreamRestriction, AvcNalUnitType, concatAvcNalUnits, deserializeAvcDecoderConfigurationRecord, determineVideoPacketType, extractNalUnitTypeForAvc, extractNalUnitTypeForHevc, HevcNalUnitType, iterateAvcNalUnits, iterateHevcNalUnits, parseAvcSps, sanitizeHevcPacketForChromium, serializeAvcDecoderConfigurationRecord, } from './codec-data.js';
...
import { assert, assertNever, CallSerializer, clamp, colorSpaceIsComplete, getInt24, getUint24, insertSorted, isChromium, isFirefox, isNumber, isWebKit, last, mapAsyncGenerator, missingWebCodecsClassMessage, promiseWithResolvers, removeItem, toAsyncIterator, toDataView, toUint8Array, validateAnyIterable, } from './misc.js';
...
export class VideoSampleSink extends BaseMediaSampleSink {
    ...
    async _createDecoder(onSample, onError) {
        if (!(await this._track.canDecode())) {
            ...
            throw new Error('This video track cannot be decoded in this environment. Make sure to check decodability before using'
                + ' a track.');
        }
...
export class AudioSampleSink extends BaseMediaSampleSink {
    ...
    async _createDecoder(onSample, onError) {
        if (!(await this._track.canDecode())) {
            ...
            throw new Error('This audio track cannot be decoded in this environment. Make sure to check decodability before using'
                + ' a track.');
        }
        const codec = await this._track.getCodec();

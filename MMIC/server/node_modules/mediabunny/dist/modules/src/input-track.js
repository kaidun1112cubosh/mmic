...
import { assert, extractRotationFromMatrix, isNumber, isThenable, matrixIsFlipped, roundToDivisor, simplifyRational, } from './misc.js';
...
export class InputVideoTrack extends InputTrack {
    ...
    async computeFrameRateMetrics(options = {}) {
        ...
        if (options.targetPacketCount !== undefined
        ...
        for await (const packet of sink.packets(undefined, undefined, { metadataOnly: true })) {
            if (timestamps.length >= targetPacketCount
        ...
        const medianFrameRate = (unitRate / medianDifferenceA
            + unitRate / medianDifferenceB) / 2;
        return {
            underlyingFrameRate,
            ...
            medianFrameRate,
            frameRateIsConstant: underlyingFrameRate !== null
                && minDifference === 1
                && maxDifference === 1,
            probedPacketCount,
        };
 ...
 */
export const findUnderlyingFrameRate = (ticks, resolution) => {
    ...
    for (let iteration = 0; iteration < 6; iteration++) {
        ...
        if (Math.abs(refinedPeriod - period)
    ...
    for (const gap of gaps) {
        ...
        if (Math.abs(gap - multiple * finalPeriod)
            < DELTA_TOLERANCE) {
            finalInlierCount++;
        }
    }
